...
cv::Vec3d phg::Calibration::project(const cv::Vec3d &point) const
{
    ...
    double r = x*x + y*y;
    double adjusted_d = 1.0+ k1_ * r + k2_ *r*r;
    x*=adjusted_d;
    y*=adjusted_d;
...
cv::Vec3d phg::Calibration::unproject(const cv::Vec2d &pixel) const
{
    ...
    for (int i =0; i<50; i++){
        ...
        double r = x_p*x_p + y_p*y_p;
        double adjusted_d = 1.0+ k1_ * r + k2_ *r*r;
            double x_n = x /adjusted_d;
            double y_n = y /adjusted_d;
        double diff = std::max(std::abs(x_p - x_n), std::abs(y_p - y_n));
